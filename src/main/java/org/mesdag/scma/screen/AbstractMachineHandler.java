...
public abstract class AbstractMachineHandler extends ScreenHandler {
    protected final Inventory inventory;
    protected final PropertyDelegate propertyDelegate;

    public AbstractMachineHandler(@Nullable ScreenHandlerType<?> type, int syncId, int expectedSize, PlayerInventory playerInventory, Inventory inventory, PropertyDelegate propertyDelegate) {
        super(type, syncId);
        checkSize(inventory, expectedSize);
        this.inventory = inventory;
        this.propertyDelegate = propertyDelegate;
        inventory.onOpen(playerInventory.player);
        addSlot(new ChargerableSlot(inventory, 0, 142, 8));
        // 玩家物品栏与快捷栏
        for (int m = 0; m < 3; ++m) {
            for (int l = 0; l < 9; ++l) {
